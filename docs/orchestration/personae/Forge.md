...
- *May 2026: Implemented Local Cover Persistence. Configured MEDIA_URL/ROOT, added `cover_image` to `Book` model, and established "Single Pull" logic in `services.py` to download and cache cover images locally while maintaining resilience against download failures. (Audited by Sentry: Backend verified; frontend prioritization and placeholders verified; **RED LIGHT** due to missing `loading="lazy"` on some components).*
