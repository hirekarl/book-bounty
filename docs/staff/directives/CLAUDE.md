...
// check section 14: CLAUDE.md (project status / migration count)
```
...
Delegate to Scout with: `"Act as Scout. Run the Pre-Release Checklist from docs/staff/personae/Scout.md against the current repo state. Report each item as ✓ / ✗ / ⚠."` Scout's checklist is the gate — no deploy until all items are ✓ or ⚠.
