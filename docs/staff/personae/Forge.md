...
- **Database-level constraints require database-level verification** — Model factories and API tests often mask schema gaps by auto-filling required fields. Explicitly test for `IntegrityError` (e.g., `user=None` on a non-nullable FK) to ensure the DB engine is the final arbiter of multi-tenant isolation.
